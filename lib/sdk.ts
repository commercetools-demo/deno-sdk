		...
		if (sdk.instance === undefined) {
			...
			if (options?.passwordflow !== undefined) {
				if (options.passwordflow.storeKey === undefined) {
					apiRoot = new sdkClient(config, verbose).withUsernamePassword(
						options,
					)
				} else {
					apiRoot = new sdkClient(config, verbose)
						.withUsernamePasswordStore(
							options,
						)
				}
			} else if (options?.anonymous !== undefined) {
				if (options.anonymous.refresh_token !== undefined) {
					apiRoot = new sdkClient(config, verbose).withRefreshToken(
						options,
					)
				} else {
