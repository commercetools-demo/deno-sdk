...
export class ApiExtension implements iApiExtension {
   ...
   async register(destination: string): Promise<boolean> {
      ...
      console.log(ctcol.yellow(`registering API extensions for commercetools project: ${this._handle.config.project_key}`))
      for (const trigger of this._triggers) {
         console.log(ctcol.yellow(`${trigger.actions.join(", ").toLowerCase()} on ${trigger.resource}`))
      }
   ...
   private async registerExtension(draft: ExtensionDraft): Promise<Extension | undefined> {
      try {
         ...
         if (_error.statusCode === 404) {  // extension does not exist yet
            ...
            console.log(ctcol.yellow(`registered extension with key ${hext.body.key}`))
            return hext.body
         }
   ...
   private async registerExtensions(destination: string, triggers: iTriggers[]) {
   ...
   private async unregisterExtensions(triggers: iTriggers[]) {
      for (const trigger of triggers) {
         ...
         await this._handle
            ...
            .execute()
         console.log(ctcol.yellow(`unregistered extension with key ${existing.body.key}`))
      }
   }
}