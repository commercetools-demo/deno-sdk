import { iMessageHandler, iReferenceMessage, iHandledResponse, responseCode } from "./iBaseHandler.ts";
...
export abstract class BaseHandler {
   ...
   async handleMessage(msg: iReferenceMessage): Promise<iHandledResponse> {
      ...
      const actions = response.filter(resp => resp.result.code === responseCode.SUCCESS && resp.result.actions?.length)
      if (actions.length) {
         const allactions =  {
         ...
         return {
            result: {
               ...
               actions: [...new Set(allactions.actions.flatMap((o) => JSON.stringify(o)))].flatMap((s) => JSON.parse(s))
            }
         }
      }
