...
export class Config {
	...
	static init(options?: iConfig): iConfig {
		if (options === undefined) {
			...
			config.import_url = (Deno.env.get("CTP_IMP_URL") !== undefined)
				? Deno.env.get("CTP_IMP_URL")
				: Config.constructImportURL(config.api_url)
			config.history_url = (Deno.env.get("CTP_HST_URL") !== undefined)
				? Deno.env.get("CTP_HST_URL")
				: Config.constructHistoryURL(config.api_url)
