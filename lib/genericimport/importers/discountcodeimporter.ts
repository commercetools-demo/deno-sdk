import { DiscountCodeImport, ImportContainer, ImportOperationStatus, importsdk } from "../../../importsdk.ts";
import { ImportType, iImportHandler } from "./iImportHandler.ts";
import { test } from "./utils.ts";

export const DiscountCode: iImportHandler = {
   importer: async (handle: importsdk, container: ImportContainer, payload: DiscountCodeImport[]): Promise<ImportOperationStatus[]> => {
      let result
      try {
         result = await handle
            .root()
   ...
   validator: (payload: DiscountCodeImport[], importtype: ImportType): boolean => {
      let valid = true
