import { ProductTypeImport, ImportContainer, ImportOperationStatus, importsdk } from "ct/importsdk";
import { ImportType, iImportHandler } from "./iImportHandler.ts";
import { test } from "./utils.ts";

export const ProductType: iImportHandler = {
   importer: async (handle: importsdk, container: ImportContainer, payload: ProductTypeImport[]): Promise<ImportOperationStatus[]> => {
      let result
      try {
         result = await handle
            .root()
   ...
   validator: (payload: ProductTypeImport[], importtype: ImportType): boolean => {
      let valid = true
