import { ProductVariantImport, ImportContainer, ImportOperationStatus, importsdk } from "ct/importsdk";
import { ImportType, iImportHandler } from "./iImportHandler.ts";
import { test } from "./utils.ts";

export const ProductVariant: iImportHandler = {
   importer: async (handle: importsdk, container: ImportContainer, payload: ProductVariantImport[]): Promise<ImportOperationStatus[]> => {
      let result
      try {
         result = await handle
            .root()
