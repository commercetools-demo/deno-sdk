import { ProductVariantImport, ImportContainer, ImportOperationStatus, importsdk, ProductVariantPatch } from "../../../importsdk.ts";
import { ImportType, iImportHandler } from "./iImportHandler.ts";
import { test } from "./utils.ts";

export const ProductVariantPatches: iImportHandler = {
   importer: async (handle: importsdk, container: ImportContainer, payload: ProductVariantPatch[]): Promise<ImportOperationStatus[]> => {
      let result
      try {
         result = await handle
            .root()
   ...
   validator: (payload: ProductVariantImport[], importtype: ImportType): boolean => {
      let valid = true
